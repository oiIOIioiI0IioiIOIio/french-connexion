---
date_creation_note: '2026-03-07'
date_fondation: ''
description: 'Une organisation est en sciences sociales un groupe social formé d''individus
  en interaction, ayant un but collectif, mais dont les préférences, les informations,
  les intérêts et les connaissances peuvent diverger : une entreprise, une administration
  publique, un syndicat, un parti politique, une association, etc.

  L''organisation de quelque chose désigne l''action d''organiser (structurer, délimiter,
  agencer, répartir ou articuler). En ce sens, il s''agit d''un processus social.

  Une organisation est le résultat d''actions réglementées (une entreprise, un service
  public, une administration, une association, une armée, un événement, etc.).


  '
domaine: politique
found_via: Rothschild
membres: []
nom: Organisation
search_depth: 0
siege: ''
sources:
- https://fr.wikipedia.org/wiki/Organisation
- https://www.wikidata.org/wiki/Q43229
statut_note: verifie_wikipedia
tags:
- institution
- elite
- niveau-0
- oeil-de-dieu
type: institution
type_organisation: association
verified: true
wikidata_id: Q43229
wikipedia_enriched: true
---

>  **Sujet principal de la recherche**


## Description

Une organisation est en sciences sociales un groupe social formé d'individus en interaction, ayant un but collectif, mais dont les préférences, les informations, les intérêts et les connaissances peuvent diverger : une entreprise, une administration publique, un syndicat, un parti politique, une association, etc.
L'organisation de quelque chose désigne l'action d'organiser (structurer, délimiter, agencer, répartir ou articuler). En ce sens, il s'agit d'un processus social.
Une organisation est le résultat d'actions réglementées (une entreprise, un service public, une administration, une association, une armée, un événement, etc.).






## Sources

- [Wikipedia](https://fr.wikipedia.org/wiki/Organisation)
- [Wikidata](https://www.wikidata.org/wiki/Q43229)


---

## Metadonnees

**Type** : Institution / Organisation  
**Categorie** : association  
**Fondation** : N/A  
**Siege** : N/A  
**Domaine** : politique  
**Statut de verification** :  Verifie  
**Date d'ajout** : 2026-03-07  

*Fiche generee -- exploration recursive niveau 0*